...
class PaperBook:
    ...
    def pages(self, value):
        if isinstance(value. int): # проверка, что число страниц целое
            if value > 0: # проверка, что число страниц больше нуля
                self.pages = value # установка значения через setter
            else:
                raise ValueError(f'Invalid number of pages: {value!r}')
        else:
            raise ValueError(f'Number of pages must be int, while input:{value!r} ')

    def __str__(self): # перезагрузка для вывода доп. параметра
    ...
    def __repr__(self): # для бумажных книг есть параметр - число страниц, нужна перезагрузка
...
class AudioBook:
    ...
    def __repr__(self): # для аудиокниг есть параметр - длительность, поэтому нужна перезагрузка
