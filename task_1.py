...
class Film:
    ...
    def add_comm(self, message: str):
        ...
        Примеры:
        >>> test_film = Film("Альфа Центавра", 158) #Инициализация объекта класса
        >>> test_film.add_comm("Интересный научнофантастический фильм")
        """
        self._comm = message
    ...
    def check_accordance(self):
        """
        Метод, печатающий степень соответствия фильма теме.
        Предварительно необходимо добавить её к экземпляру класса
        Примеры:
        >>> test_film = Film("Альфа Центавра", 158)
        >>> test_film.add_accordance(9.)
...
class HistoricalFilm(Film):
    ...
    def check_accordance(self): # методу необходима перегрузка, чтобы показывать степень исторического соответствия
        """
        Метод, печатающий степень исторического соответствия фильма, ранее добавленную методом add_accordance
        Примеры:
        >>> test_film = HistoricalFilm("Александр Невский", 149, "XIII")
...
if __name__ == "__main__":
    ...
    Возможные проверки на соответствие века формату цифр опущены, так как век может быть указан с модификатором до н.э или н.э.
    """
    doctest.testmod()
    pass
